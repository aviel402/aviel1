# קובץ: api/index.py
# גרסה מותאמת לסביבת Vercel, הכוללת ראוטר ושתי אפליקציות

from flask import Flask, request, Response
import datetime

# שם המשתנה 'app' הוא חובה עבור Vercel
app = Flask(__name__)
...
def handle_time(params):
    yemot_commands = []
    tz = datetime.timezone(datetime.timedelta(hours=3)) # שעון ישראל (קיץ)
...
# --- הנתב הראשי ---
# הראוטר מאזין בנתיב הראשי של האפליקציה
...
def main_router():
    try:
        ...
        print(f"ERROR: {e}")
        yemot_commands = ["id_list_message=t-אירעה שגיאה בשרת"]
